...
# Additional distributions: below 10% and above 180%
st.sidebar.header("Additional distributions")
below_mean = st.sidebar.number_input(
    "Below 10% dist mean",
    value=0.0,
    format="%.2f"
)
below_std = st.sidebar.number_input(
    "Below 10% dist std dev",
    min_value=0.0,
    value=1.0,
    format="%.2f"
)
below_min = st.sidebar.number_input(
    "Below 10% dist min",
    value=0.0,
    format="%.2f"
)
below_max = st.sidebar.number_input(
    "Below 10% dist max",
    value=0.0,
    format="%.2f"
)
...
below_data = np.random.normal(loc=below_mean, scale=below_std, size=n)
below_data = np.clip(below_data, below_min, below_max)
above_data = np.random.normal(loc=above_mean, scale=above_std, size=n)
above_data = np.clip(above_data, above_min, above_max)

# --- Plotting ---
fig, ax = plt.subplots()
ax.hist(data, bins=50, alpha=0.7, edgecolor='white', label='Primary')
ax.hist(below_data, bins=50, alpha=0.3, edgecolor='white', label='<10%')
ax.hist(above_data, bins=50, alpha=0.3, edgecolor='white', label='>180%')
